...
const LearningSection = () => {
  ...
  interface CourseData {
    learnings: string[];
    courseContent: { chapter: string; details: string[] }[];
  }
  const [selectedCourse, setSelectedCourse] = useState<CourseData>({learnings:[], courseContent:[]});
  ...
  useEffect(() => {
    const fetchData = async () => {
      if (message) {
        const courseName:any = message ; // Explicitly cast message to CourseName
        const courseData:any = await getCourseData(courseName);
